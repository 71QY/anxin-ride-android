...
import com.example.myapplication.data.model.TravelRecord  // ⭐ 新增：出行记录
import com.example.myapplication.data.model.TravelRecordsResponse  // ⭐ 新增：出行记录响应
...
interface ApiService {
    ...
    /**
     * ⭐ 新增：分享收藏地点给亲友（简化版，直接传参）
     * POST /api/favorites/share-to-guardian
     */
    @FormUrlEncoded
    @POST("favorites/share-to-guardian")
    suspend fun shareFavoriteToGuardian(
        @Field("favoriteId") favoriteId: Long,
        @Field("guardianUserId") guardianUserId: Long
    ): Result<Unit>
    ...
    /**
     * ⭐ 新增：确认到达目的地（简化版，直接传参）
     * POST /api/favorites/confirm-arrival-simple
     */
    @FormUrlEncoded
    @POST("favorites/confirm-arrival-simple")
    suspend fun confirmArrival(
        @Field("favoriteId") favoriteId: Long,
     ...
     * POST /api/favorites/share-to-elder
     */
    @POST("favorites/share-to-elder")
    suspend fun shareFavoriteToElder(
        @Body request: ShareFavoriteRequest
    ): Result<Unit>
     ...
     */
    @GET("travel-records")
    suspend fun getTravelRecords(
        @Query("page") page: Int = 1,
        @Query("size") size: Int = 10,
        @Query("startDate") startDate: String? = null,
        @Query("endDate") endDate: String? = null
    ): Result<TravelRecordsResponse>
}
